...
- Solo Admin borra directo (`session.get('rol') == 'admin'`); Admin y operador pueden *solicitar* — `requiere_personal()` los admite a ambos para entrar al flujo, pero la rama de borrado inmediato está gateada aparte.
...
- **`requiere_personal()` no restringe a admin.** Pese a lo que su nombre anterior (`requiere_admin()`) sugería, permite `admin` **y** `operador` por igual (`routes/admin.py`) — es en realidad un "¿está logueado como staff?", no un gate de admin exclusivo. Si una ruta o botón debe ser admin-only de verdad, usa `solo_admin()` (también en `routes/admin.py`, importable desde ahí) además de (o en vez de) `requiere_personal()`. No asumas que una pantalla protegida por `requiere_personal()` es invisible para operador.
