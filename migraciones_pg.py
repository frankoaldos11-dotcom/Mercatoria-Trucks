...
def ejecutar_migraciones_pg():
    ...
    # La verificación de existencia del schema ocurre SIEMPRE primero, sin importar
    # SKIP_MIGRATIONS: una base vacía debe crearse siempre, con o sin el flag activo.
    cur.execute("""
        SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'usuarios'
    """)
    schema_existe = cur.fetchone()[0] > 0
    ...
    # A partir de aquí el schema ya existe: SKIP_MIGRATIONS solo puede saltar las
    # migraciones incrementales, nunca la creación inicial (ya cubierta arriba).
    if os.environ.get("SKIP_MIGRATIONS") == "true":
        print("[migraciones_pg] SKIP_MIGRATIONS=true — omitiendo migraciones incrementales (schema ya existe).")
        conn.close()
        return
